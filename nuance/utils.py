...
async def http_request_with_retry(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            bt.logging.warning(f"⚠️  Attempt {attempt + 1} for {url} failed: {e}")
            if attempt < MAX_RETRIES - 1:
                await asyncio.sleep(RETRY_DELAY * (attempt + 1))
            else:
                bt.logging.error(f"❌ All {MAX_RETRIES} attempts failed for {url}.")
                raise
...
def record_db_error(db: shelve.Shelf, error_msg: str) -> None:
    ...
    bt.logging.debug(f"💾 Recorded error: {error_msg}")